...
import pgasync as Database
from tx_green import wait, wait_decor
...
class CursorWrapper(object):
    ...
    def __getattribute__(self,name):
        if name in ['cursor','__class__','__iter__']:
            return object.__getattribute__(self,name)
        attr = object.__getattribute__(self.cursor,name)
        if callable(attr):
            return wait_decor(attr)
        return attr
...
class DatabaseWrapper(BaseDatabaseWrapper):
    ...
    def _enter_transaction_management(self, managed):
        ...
        if self.features.uses_autocommit and managed and not self.isolation_level:
            pass #self._set_isolation_level(1)
    ...
    def _leave_transaction_management(self, managed):
        ...
        if self.features.uses_autocommit and not managed and self.isolation_level:
            pass #self._set_isolation_level(0)
